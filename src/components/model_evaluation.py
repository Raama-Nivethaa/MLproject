...
class ModelEvaluation:
    def __init__(self):
        logging.info("Heart Disease Model Evaluation Started")
    ...
    def initiate_model_evaluation(self, train_array, test_array):
        try:
            ...
            with mlflow.start_run():
                ...
                logging.info(f"Metrics logged: Accuracy={accuracy}, Precision={precision}, Recall={recall}, F1-Score={f1}")

                if tracking_url_type_store != "file":
                    mlflow.sklearn.log_model(model, "model", registered_model_name="heart_disease_model")
                else:
